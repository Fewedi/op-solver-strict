...
class ProblemWriterWithDummy: ProblemWriter {
    ...
    private fun createDistanceMatrix(cluster: Cluster, distanceMatrix: Array<DoubleArray>, nodeList: MutableList<Node>): Array<DoubleArray> {
        ...
        if (ConfigProvider.config.algorithm.ea4op!!.dummyStartNode == DummyStartNode.CLUSTER){
            nodeList.forEach(){ node ->
                clusterDistanceMatrix[0][node.writtenPosition] = node.distanceToNextCluster * ConfigProvider.config.algorithm.ea4op!!.dummyStartNodeFactor
                clusterDistanceMatrix[node.writtenPosition][0] = node.distanceToPrevCluster * ConfigProvider.config.algorithm.ea4op!!.dummyStartNodeFactor
            }
        }
